...
describe Tapyrus::Util do
  ...
  describe "pack and unpack" do
    ...
    it "should pack var int" do
      ...
      expect(util.pack_var_int(65_535)).to eq([0xfd, 0xff, 0xff].pack("C*"))
      expect(util.pack_var_int(65_536)).to eq([0xfe, 0x00, 0x00, 0x01, 0x00].pack("C*"))
    end
    ...
    it "should unpack var int" do
      ...
      expect(util.unpack_var_int([0xfd, 0xff, 0xff].pack("C*")).first).to eq(65_535)
      expect(util.unpack_var_int([0xfe, 0x00, 0x00, 0x01, 0x00].pack("C*")).first).to eq(65_536)
      expect(util.unpack_var_int([0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff].pack("C*")).first).to eq(
        0xffffffffffffffff
      )
    ...
    it "should unpack var int from io" do
      ...
      expect(util.unpack_var_int_from_io(StringIO.new([0xfd, 0xff, 0xff].pack("C*")))).to eq(65_535)
      expect(util.unpack_var_int_from_io(StringIO.new([0xfe, 0x00, 0x00, 0x01, 0x00].pack("C*")))).to eq(65_536)
      expect(
        util.unpack_var_int_from_io(StringIO.new([0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff].pack("C*")))
  ...
  describe "#hash160" do
    it "should be hashed" do
      expect(util.hash160("0292ee82d9add0512294723f2c363aee24efdeb3f258cdaf5118a4fcf5263e92c9")).to eq(
  ...
  describe "#decode_base58_address" do
    context "p2pkh address" do
      subject { util.decode_base58_address("mmy7BEH1SUGAeSVUR22pt5hPaejo2645F1") }
      it "should be encoded" do
    ...
    context "p2sh address" do
      subject { util.decode_base58_address("2N3wh1eYqMeqoLxuKFv8PBsYR4f8gYn8dHm") }
      it "should be encoded" do
    ...
    context "cp2pkh address" do
      ...
      it "should be encoded" do
        expect(subject[0]).to eq(
          "c3ec2fd806701a3f55808cbec3922c38dafaa3070c48c803e9043ee3642c660b4646c2fbfbecc99a63148fa076de58cf29b0bcf0b0"
        )
    ...
    context "cp2sh address" do
      ...
      it "should be encoded" do
        expect(subject[0]).to eq(
          "c3ec2fd806701a3f55808cbec3922c38dafaa3070c48c803e9043ee3642c660b467620a79e8657d066cff10e21228bf983cf546ac6"
        )
  ...
  describe "#valid_address?" do
    context "prod", network: :prod do
      it "should be true" do
        ...
        # CP2PKH
        expect(
          util.valid_address?("w26x2EaheVBsceNf9RufpmmZ1i1qLBux1UMKMs16dkcZxTP8FBRDXGQ3Cim71aJ1gtoFNttSPNfLsC")
        ).to be true

        # CP2SH
        expect(
          util.valid_address?("4a28F5ZehQNaMsSCEzBGQSKjVx2Wz2c4s32joimPciFTLzc7AUqsfg2xhoBq8NAjEpRNFNUrAZrpEHB")
        ).to be true
        ...
        expect(
          util.valid_address?("22VdQ5VjWcF9zgsnPQodFBS1PBQPaAQEXSofkyMv2D9zV1MLp3JfScV6TMVaUQ42xeTfjieWssAaefMd")
        ).to be false
        expect(
          util.valid_address?("2oLdn5UKgY7DayDDLL6LKfrNnHKp7iFK8zGAMHVGd2USnCxi3XmHdMBjrPdXXsoJUCn3R4J1RfbFP2aW")
        ).to be false
      end
    end
