...
describe Tapyrus::Mnemonic, network: :mainnet do
  ...
  describe '#languages' do
    it 'should be list support language' do
      expect(Tapyrus::Mnemonic.word_lists).to match_array(%w(english japanese french spanish chinese_simplified chinese_traditional italian))
    end
  end
    ...
    context 'japanese' do
      ...
      it 'pass test vectors' do
        ...
        vectors.each do |vector|
          ...
          xprv = Tapyrus::ExtKey.generate_master(subject.to_seed(vector['mnemonic'].split('　'), passphrase: vector['passphrase']))
