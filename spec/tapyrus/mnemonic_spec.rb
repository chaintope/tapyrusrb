...
describe Tapyrus::Mnemonic, network: :prod do
  ...
  describe '#initialize' do
    it 'should check supported words' do
      ...
      expect { Tapyrus::Mnemonic.new('english1') }.to raise_error(ArgumentError)
    end
  end

  describe '#languages' do
    it 'should be list support language' do
      expect(Tapyrus::Mnemonic.word_lists).to match_array(
        %w[english japanese french spanish chinese_simplified chinese_traditional italian]
      )
    end
  end
  ...
  describe '#to_mnemonic' do
    it 'should be generate' do
      expect(subject.to_mnemonic('c10ec20dc3cd9f652c7fac2f1230f7a3c828389a14392f05')).to eq(
        %w[
          scissors
          invite
          lock
  ...
  describe '#to_entropy' do
    context 'lower case' do
      it 'should be generate' do
        ...
        ).to eq('c10ec20dc3cd9f652c7fac2f1230f7a3c828389a14392f05')
      end
    end
    ...
    context 'checksum mismatch' do
      it 'should raise error' do
        expect {
          subject.to_entropy(%w[letter advice cage absurd amount doctor acoustic avoid letter advice cage outside])
  ...
  describe '#to_seed' do
    context 'lower case' do
      it 'should be generate' do
        expect(
          subject.to_seed(
            %w[letter advice cage absurd amount doctor acoustic avoid letter advice cage above],
            passphrase: 'TREZOR'
          )
        ).to eq(
    ...
    context 'checksum mismatch' do
      it 'should raise error' do
        expect {
          subject.to_seed(%w[letter advice cage absurd amount doctor acoustic avoid letter advice cage outside])
        }.to raise_error('checksum mismatch.')
      end
    end
  end
  ...
  describe 'Test Vectors' do
    context 'english' do
      it 'pass test vectors' do
        ...
        vectors.each do |entropy, mnemonic, seed, ext_priv_key|
          ...
          xprv = Tapyrus::ExtKey.generate_master(subject.to_seed(mnemonic.split(' '), passphrase: 'TREZOR'))
    ...
    context 'japanese' do
      subject { Tapyrus::Mnemonic.new('japanese') }
      it 'pass test vectors' do
        ...
        vectors.each do |vector|
          ...
          expect(subject.to_seed(vector['mnemonic'].split('　'), passphrase: vector['passphrase'])).to eq(
            vector['seed']
          )
          xprv =
            Tapyrus::ExtKey.generate_master(
              subject.to_seed(vector['mnemonic'].split('　'), passphrase: vector['passphrase'])
            )
