...
describe Tapyrus::Message::Version do
  ...
  describe 'to_payload and parse_from_payload' do
    subject {
      Tapyrus::Message::Version.parse_from_payload('721101000100000000000000bc8f5e5400000000010000000000000000000000000000000000ffffc61b6409208d010000000000000000000000000000000000ffffcb0071c0208d128035cbc97953f80f2f5361746f7368693a302e392e332fcf05050001'.htb)
    }
  ...
  describe 'specify opts' do
    subject {
      Tapyrus::Message::Version.new(start_height: 500, remote_addr: '83.243.59.57:8333')
    }
  ...
  describe 'to_pkt' do
    subject {
      Tapyrus::Message::Version.new(local_addr: Tapyrus::Message::NetworkAddr.new(port: 18333),
                                    remote_addr: Tapyrus::Message::NetworkAddr.new(port: 18333),
    ...
    it 'should generate pkt' do
      expect(subject.bth).to eq('0b11090776657273696f6e0000000000670000004a6a37257f1101000000000000000000cf31455900000000000000000000000000000000000000000000ffff7f000001479d000000000000000000000000000000000000ffff7f000001479d40abec703bf6eeba112f7461707972757372623a302e312e302f0000000000')
    end
  end
