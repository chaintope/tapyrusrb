...
describe Tapyrus::Wallet::Account do
  ...
  describe '#parse_from_payload' do
    subject {
      # m/84'/1'/10'
      Tapyrus::Wallet::Account.parse_from_payload('045f1cf6030ef4b1af8000000ad25ad4af81ddaf5f9577f082c333976c67038e0ab1674c2f1ae2c3f531f7198d0352b292231392201da3861e4004ffe41b1432de6f8545a080dde2988b8b8a716b09e38386e382b9e38388540000000a00000001000000010000000a000000'.htb)
    }
  ...
  describe '#new' do
    ...
    it 'should check purpose and account key are inconsistent.' do
      expect{Tapyrus::Wallet::Account.new(subject, Tapyrus::Wallet::Account::PURPOSE_TYPE[:native_segwit], 3)}.to raise_error('The purpose and the account key do not match.')
      expect{Tapyrus::Wallet::Account.new(subject, Tapyrus::Wallet::Account::PURPOSE_TYPE[:legacy], 1)}.to raise_error('Account key and index does not match.')
    end
  end
