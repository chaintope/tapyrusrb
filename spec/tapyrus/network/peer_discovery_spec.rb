    ...
    context "dns_seeds is empty" do
      ...
      context "multiple connect nodes exist" do
        let(:configuration) do
          Tapyrus::Node::Configuration.new(connect: %w[123.123.123.123 123.123.123.124 123.123.123.125])
        end
        it "should return multiple node" do
          expect(subject.peers).to eq %w[123.123.123.123 123.123.123.124 123.123.123.125]
        end
      end
      context "duplidate nodes exist" do
        let(:configuration) { Tapyrus::Node::Configuration.new(connect: %w[123.123.123.123 123.123.123.123]) }
