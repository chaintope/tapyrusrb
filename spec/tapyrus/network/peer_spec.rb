...
describe Tapyrus::Network::Peer do
  ...
  subject do
    ...
    peer =
      Tapyrus::Network::Peer.new(
        "210.196.254.100",
        18_333,
        Tapyrus::Network::Pool.new(node_mock, chain, configuration),
        configuration
      )
  ...
  describe "#to_network_addr" do
    before do
      opts = {
        version: 70_015,
        services: 13,
        timestamp: 1_507_879_363,
        local_addr: "0.0.0.0:0",
        remote_addr: "94.130.106.254:63446",
        nonce: 1_561_841_459_448_609_851,
        user_agent: "/Satoshi:0.14.2/",
        start_height: 1_210_117,
        relay: true
      }
    ...
    it "should be generate" do
      ...
      expect(network_addr.port).to eq(18_333)
      expect(network_addr.services).to eq(13)
      expect(network_addr.time).to eq(1_507_879_363)
    end
  end
  ...
  describe "#handler_tx" do
    it "should call update with :tx" do
      listener = double("listener")
      payload =
        "".htb
