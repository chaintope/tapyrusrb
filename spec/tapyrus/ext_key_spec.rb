...
describe Tapyrus::ExtKey, network: :prod do
  describe "Test Vector 1" do
    before { @master_key = Tapyrus::ExtKey.generate_master("000102030405060708090a0b0c0d0e0f") }
    ...
    it "Chain m" do
      ...
      expect(@master_key.to_base58).to eq(
    ...
    it "Chain m/0H/1" do
      ...
      expect(ext_pubkey.to_base58).to eq(
        "xpub6BbKTokSJPVngFxgm1Y42PWHVwa3nBveH2bhhjhqN5Y5Katvs7XD7AwK3LAx9RQfdHjLNXYgMx9pav9de2VDZ3ovqZfhYsqhXdN1D8ZknGV"
      )
    ...
    it "Chain m/0H/1/2H/2/1000000000" do
      key = @master_key.derive(0, true).derive(1).derive(2, true).derive(2).derive(1_000_000_000)
  ...
  describe "Test Vector 2" do
    before do
      @master_key =
        Tapyrus::ExtKey.generate_master(
    ...
    it "Chain m/0/2147483647H" do
      key = @master_key.derive(0).derive(2_147_483_647, true)
      expect(key.depth).to eq(2)
      expect(key.hardened?).to be true
    ...
    it "Chain m/0/2147483647H/1" do
      key = @master_key.derive(0).derive(2_147_483_647, true).derive(1)
      expect(key.depth).to eq(3)
      expect(key.hardened?).to be false
    ...
    it "Chain m/0/2147483647H/1/2147483646H" do
      key = @master_key.derive(0).derive(2_147_483_647, true).derive(1).derive(2_147_483_646, true)
      expect(key.depth).to eq(4)
      expect(key.hardened?).to be true
    ...
    it "Chain m/0/2147483647H/1/2147483646H/2" do
      key = @master_key.derive(0).derive(2_147_483_647, true).derive(1).derive(2_147_483_646, true).derive(2)
      expect(key.depth).to eq(5)
      expect(key.hardened?).to be false
      ...
      ext_pubkey =
        @master_key.derive(0).derive(2_147_483_647, true).derive(1).derive(2_147_483_646, true).ext_pubkey.derive(2)
      expect(ext_pubkey.to_base58).to eq(
        "xpub6Gm4ZoEgAPL6LHPmFUDmmmR2DGcN1B7LLhjTmC8WrsKPLcPtsFrQntthobzSvu7t8uiBD1GkK6jek6Life3QvWygRYkAegjuFNTK1v1F858"
      )
      expect(key.ext_pubkey.hardened?).to be false
  ...
  describe "import from base58 address" do
    it "import private key" do
      ...
      }.to raise_error("An unsupported version byte was specified.")
    end
  ...
  describe "#parse_from_payload" do
    it "should deserialize key object." do
      ext_pubkey =
        Tapyrus::ExtPubkey.parse_from_payload(
          "0488b21e0354f9b99e8000000204466b9cc8e161e966409ca52986c584f07e9dc81f735db683c3ff6ec7b1503f0357bfe1e341d01c69fe5654309956cbea516822fba8a601743a012a7896ee8dc2".htb
        )
      expect(ext_pubkey.to_base58).to eq(
        "xpub6CGzenk449cm5VqBBH4iCJtAJysSRDt47eJGKuhdn1t1wWjv4bz1ZEUGjPqaa3wVPRcBm1nMZ956Tkc64mzAhAjZbXw1dAGfqLWm994LYxG"
      )

      key =
        Tapyrus::ExtKey.parse_from_payload(
          "0488ade4025c1bd648000000012a7857631386ba23dacac34180dd1983734e444fdbf774041578e9b6adb37c19003c6cb8d0f6a264c91ea8b5030fadaa8e538b020f0a387421a12de9319dc93368".htb
        )
      expect(key.to_base58).to eq(
        "xprv9wTYmMFdV23N2TdNG573QoEsfRrWKQgWeibmLntzniatZvR9BmLnvSxqu53Kw1UmYPxLgboyZQaXwTCg8MSY3H2EU4pWcQDnRnrVA1xe8fs"
      )
  ...
  def test_vector4
    ...
    }.to raise_error(ArgumentError, Tapyrus::Errors::Messages::INVALID_PUBLIC_KEY)
    ...
    }.to raise_error(ArgumentError, Tapyrus::Errors::Messages::INVALID_BIP32_PRIV_PREFIX)
    ...
    }.to raise_error(ArgumentError, Tapyrus::Errors::Messages::INVALID_BIP32_FINGERPRINT)
    expect {
      Tapyrus::ExtKey.from_base58(
    ...
    }.to raise_error(ArgumentError, Tapyrus::Errors::Messages::INVALID_BIP32_ZERO_INDEX)
    expect {
      Tapyrus::ExtKey.from_base58(
    ...
    expect {
      Tapyrus::ExtPubkey.from_base58(
        "xpub67tVq9TC3jGc6K4by6z6kdQafQSuot6u4B8hWn5Jd6vh9hdKNusPnNU4r2sXEKbgYVAAkbLpeut3DMSgLDAvSEQFHEp3f2MJaNQRiCpcWj3"
      )
    }.to raise_error(ArgumentError, Tapyrus::Errors::Messages::INVALID_BIP32_ZERO_DEPTH)
    expect {
      Tapyrus::ExtKey.from_base58(
        "xprv9tu9RdvJDMiJspz8s5T6PVTr7NcRQRP3gxD6iPfh4mPiGuJAqNZ9Ea9aziKNptLTaB28LkiTWqvg636gvKqKxoVtLSVj2tUDqBzHxUKKS2m"
      )
    }.to raise_error(ArgumentError, Tapyrus::Errors::Messages::INVALID_BIP32_ZERO_DEPTH)
    ...
    }.to raise_error(ArgumentError, Tapyrus::Errors::Messages::INVALID_BIP32_VERSION)
    expect {
      Tapyrus::ExtKey.from_base58(
    ...
    }.to raise_error(ArgumentError, Tapyrus::Errors::Messages::INVALID_CHECKSUM)
  end
end
