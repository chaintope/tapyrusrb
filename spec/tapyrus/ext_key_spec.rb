...
describe Tapyrus::ExtKey, network: :mainnet do
  ...
  describe 'bip 49' do
    ...
    context 'testnet', network: :testnet do
      it 'should be used bip44 encoding' do
        ...
        expect(receive_key.key_type).to eq(Tapyrus::Key::TYPES[:p2wpkh_p2sh])
      end
    end
  end
  ...
  describe 'bip 84' do
    ...
    it 'should be changed version bytes' do
      ...
      expect(receive_key.key_type).to eq(Tapyrus::Key::TYPES[:p2wpkh])
      ...
      expect(receive_key2.key_type).to eq(Tapyrus::Key::TYPES[:p2wpkh])
      ...
      expect(change_key.key_type).to eq(Tapyrus::Key::TYPES[:p2wpkh])

      # recover from xprv
      ext_prv = Tapyrus::ExtKey.from_base58('zprvAdG4iTXWBoARxkkzNpNh8r6Qag3irQB8PzEMkAFeTRXxHpbF9z4QgEvBRmfvqWvGp42t42nvgGpNgYSJA9iefm1yYNZKEm7z6qUWCroSQnE')
      ...
      expect(ext_prv.key_type).to eq(Tapyrus::Key::TYPES[:p2wpkh])

      # recover from xpub
      ext_pub = Tapyrus::ExtPubkey.from_base58('zpub6rFR7y4Q2AijBEqTUquhVz398htDFrtymD9xYYfG1m4wAcvPhXNfE3EfH1r1ADqtfSdVCToUG868RvUUkgDKf31mGDtKsAYz2oz2AGutZYs')
      ...
      expect(ext_pub.key_type).to eq(Tapyrus::Key::TYPES[:p2wpkh])
    end
  end
