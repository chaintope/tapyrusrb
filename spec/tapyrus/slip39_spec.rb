...
describe Tapyrus::SLIP39 do
  ...
  describe "test_basic_sharing_fixed" do
    it "should reconstruct same secret." do
      group = Tapyrus::SLIP39::SSS.setup_shares(group_threshold: 1, groups: [[3, 5]], secret: master_secret)
      expect(group.length).to eq(1)
      expect(group.first.length).to eq(5)
      s1 = Tapyrus::SLIP39::SSS.recover_secret(group.first[0..2])
      s2 = Tapyrus::SLIP39::SSS.recover_secret(group.first[2..-1])
      expect(s1).to eq(s2)
  ...
  describe "test_group_sharing" do
    it "should generate group shares." do
      ...
      expect(Tapyrus::SLIP39::SSS.recover_secret([group_shares[2][3], group_shares[3][0], group_shares[2][4]]))
  ...
  describe "get_salt" do
    it "should encode the identifier with two bytes." do
      expect(Tapyrus::SLIP39::SSS.send(:get_salt, 5)).to eq("7368616d69720005".htb)
      expect(Tapyrus::SLIP39::SSS.send(:get_salt, 256)).to eq("7368616d69720100".htb)
    end

    it "should encode the identifier with the minimum number of bytes in legacy mode." do
      expect(Tapyrus::SLIP39::SSS.send(:get_salt, 5, true)).to eq("7368616d697205".htb)
      expect(Tapyrus::SLIP39::SSS.send(:get_salt, 256, true)).to eq("7368616d69720100".htb)
    end
  end

  describe "legacy_salt option" do
    # 1-of-1 share generated by tapyrus-ruby 0.4.0, whose identifier is 116.
    let(:words) do
      "acquire painting academic academic capital carbon legal cylinder depict award withdraw prepare guard wildlife busy velvet gray endless artist that"
    end
    let(:shares) { [Tapyrus::SLIP39::Share.from_words(words.split(" "))] }

    it "should recover the master secret." do
      expect(Tapyrus::SLIP39::SSS.recover_secret(shares, passphrase: "TREZOR", legacy_salt: true)).to eq(
        master_secret_128
      )
    end
