...
describe Tapyrus::Multisig do
  ...
  describe '#sort_p2sh_multisig_signatures' do
    ...
    let(:p2sh) { Tapyrus::Script.to_p2sh_multisig_script(m, keys.map(&:pubkey)) }
    let(:keys) { 3.times.map { Tapyrus::Key.generate } }
  ...
  describe '#sort_witness_multisig_signatures' do
    let(:tx) do
      Tapyrus::Tx.new.tap do |tx|
        out_point = Tapyrus::OutPoint.new("txid", 0)
        tx.inputs << Tapyrus::TxIn.new(out_point: out_point)
        tx.outputs << Tapyrus::TxOut.new(value: 10_000_000, script_pubkey: Tapyrus::Script.new)
      end
    end
    let(:redeem_script) { Tapyrus::Script.to_multisig_script(m, keys.map(&:pubkey)) }
    let(:keys) { 3.times.map { Tapyrus::Key.generate } }
    let(:sig_hash) { tx.sighash_for_input(0, redeem_script, amount: 20_000_000, sig_version: :witness_v0) }

    subject { tx.verify_input_sig(0, Tapyrus::Script.to_p2wsh(redeem_script), amount: 20_000_000) }
    ...
    context "3 of 3" do
      ...
      it do
        ...
        [0, 1, 2].permutation do |order|
          ...
          order.each do |i|
            Tapyrus::Multisig.add_sig_to_multisig_script_witness(keys[i].sign(sig_hash), script_witness)
          end
          script_witness.stack << redeem_script.to_payload
          Tapyrus::Multisig.sort_witness_multisig_signatures(script_witness, sig_hash)
          expect(subject).to be_truthy
        end
      end
    end
