  ...
  describe 'check script_test.json' do
    ...
    script_json.each do| r |
      it "should validate script #{r.inspect}" do
        if r[0].is_a?(Array)
          witness = Tapyrus::ScriptWitness.new(r[0][0..-2].map{ |v| v.htb })
          sig, pubkey, flags, error_code = r[1], r[2], r[3], r[4]
          amount = (r[0][-1] * 100_000_000).to_i
        else
          witness, sig, pubkey, flags, error_code = Tapyrus::ScriptWitness.new, r[0], r[1], r[2], r[3]
          amount = 0
        end
        ...
        script_flags = flags.split(',').map {|s| Tapyrus.const_get("SCRIPT_VERIFY_#{s}")}.inject(Tapyrus::SCRIPT_VERIFY_NONE){|flags, f| flags |= f}
        expected_err_code = find_error_code(error_code)
        i = Tapyrus::ScriptInterpreter.new(flags: script_flags, checker: Tapyrus::TxChecker.new(tx: tx, input_index: 0, amount: amount))
        ...
        16.times do
          ...
          next if combined_flags & Tapyrus::SCRIPT_VERIFY_CLEANSTACK && ~combined_flags & (Tapyrus::SCRIPT_VERIFY_P2SH | Tapyrus::SCRIPT_VERIFY_WITNESS)
          next if combined_flags & Tapyrus::SCRIPT_VERIFY_WITNESS && ~combined_flags & Tapyrus::SCRIPT_VERIFY_P2SH
          i = Tapyrus::ScriptInterpreter.new(flags: combined_flags, checker: Tapyrus::TxChecker.new(tx: tx, input_index: 0, amount: amount))
